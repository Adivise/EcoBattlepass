package com.exanthiax.xbattlepass.commands.helpers
...
object Messages {
    ...
    fun sendGiveUsage(sender: CommandSender) =
        sender.sendMessage("§cUsage: /xbattlepass give <xp|tiers|taskxp> <player|all> <pass> [args...]")

    fun sendSetPremiumUsage(sender: CommandSender) =
        sender.sendMessage("§cUsage: /xbattlepass setpremium <player> <pass> [true|false] [silent]")

    fun sendQuestsUsage(sender: CommandSender) =
        sender.sendMessage("§cUsage: /xbattlepass quests [pass] [category]")

    fun sendCompleteTaskUsage(sender: CommandSender) =
        sender.sendMessage("§cUsage: /xbattlepass compelete_task <player|all> <pass> <category> <quest> <task>")

    fun sendTiersUsage(sender: CommandSender) =
        sender.sendMessage("§cUsage: /xbattlepass tiers <pass>")

    fun sendResetUsage(sender: CommandSender) =
        sender.sendMessage("§cUsage: /xbattlepass reset <pass|task> <player|all> <pass> [category] [quest] [task]")

    fun sendDynamicPassUsage(sender: CommandSender) =
        sender.sendMessage("§cUsage: /<pass> [tiers|quests <category>]")
}
